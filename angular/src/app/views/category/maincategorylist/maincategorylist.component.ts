...
export class MaincategorylistComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.curentUser && this.curentUser.role == "subadmin") {
      if (this.router.url == '/app/category/category-list' && this.curentUser.privileges) {
  ...
  getdata(data) {
    this.apiService.CommonApi(Apiconfig.categoryList.method, Apiconfig.categoryList.url, data).subscribe(response => {
      if (response && response.length > 0) {
        this.categorylist = response[0];
        this.count = response[1];
        this.getcardsdata = response[2];
        this.source.load(this.categorylist);
        this.apiService.CommonApi(Apiconfig.archieveCategory.method, Apiconfig.archieveCategory.url, data).subscribe(result => {
