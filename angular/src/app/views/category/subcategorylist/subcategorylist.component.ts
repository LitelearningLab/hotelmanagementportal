...
export class SubcategorylistComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.curentUser && this.curentUser.role == "subadmin") {
      if (this.router.url == '/app/category/sub-category-list' && this.curentUser.privileges) {
  ...
  getdata(data) {
    this.apiService.CommonApi(Apiconfig.subcategoryList.method, Apiconfig.subcategoryList.url, data).subscribe(response => {
      ...
      if (response && response.length > 0) {
        ...
        this.getcardsdata = response[2];
        this.source.load(this.categorylist);
        this.apiService.CommonApi(Apiconfig.archieveSubCategory.method, Apiconfig.archieveSubCategory.url, data).subscribe(result => {
