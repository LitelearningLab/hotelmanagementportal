...
@Component({
  selector: 'app-sentancedatesenario',
  templateUrl: './sentancedatesenario.component.html',
  styleUrls: ['./sentancedatesenario.component.scss']
})
export class SentancedatesenarioComponent {
settings: any;
  ...
  pageTitle: string = 'Pronunciation Lab';
  ...
  ngOnInit(): void {
    ...
    this.ActivatedRoute.paramMap.subscribe((result)=>{
      ...
      this.senteceslabreportuser(this.userid)
    })
  

  };
  senteceslabreportuser(id:any){
    this.apiService.CommonApi(Apiconfig.sentenceslabdatesenario.method,Apiconfig.sentenceslabdatesenario.url,{userid:id,scenario:this.senarios}).subscribe((result)=>{
      console.log(result)
      if(result.status){
    ...
    if (searchTerm) {
        filteredData = this.resultdata.filter(item => {
            return (
                ...
                item.totalWords.toString().includes(searchTerm) ||
                item.totalCorrectAttempts.toString().includes(searchTerm) ||
                item.totalPracticeAttempts.toString().includes(searchTerm) ||
                item.totalListeningAttempts.toString().includes(searchTerm)
            );
        });
    }
        ...
        columns: {
          ...
          date: {
            ...
            valuePrepareFunction: (value, row) => {
              return `<a href="#/app/reports/sentenceslabreports/${this.userid}/${this.senarios}/${row.date}" style="text-decoration:none;">${value ? value : '-'}</a>`;
            }
          ...
          //   title: 'Action',
          //   type: 'custom',
          //   renderComponent: AccordionComponent,
          //   onComponentInitFunction: (instance: any) => {
          //     instance.save.subscribe(event,row => {
          //       this.toggleAccordion(event,row);
          //     });
          //   }
          // },
         
          noOfSentences: {
            title: 'No of sentences',
            filter: true,
            valuePrepareFunction: value => {
  ...
  toggleAccordion(event: Event, row: any) {
    const target = (event.target as HTMLElement).closest('td');
    if (target) {
      const accordionComponent = this.renderer.createElement('app-accordion');
      this.renderer.setAttribute(accordionComponent, 'rowData', JSON.stringify(row));
      this.renderer.appendChild(target, accordionComponent);
    }
  }
