...
@Component({
    ...
    template: `
    <span style="cursor: pointer;" title="Update as Order Packed" class='badge badge-danger badge-pill py-2 px-2' *ngIf="rowData && rowData.status == 1" (click)="confirmModal.show()" >Cancel Order</span>
    <span style="cursor: pointer;" title="Update as Ongoing Order" class='badge badge-danger badge-pill py-2 px-2' *ngIf="rowData && rowData.status == 3" (click)="confirmModal.show()" >Cancel Order</span>
    <span style="cursor: pointer;" title="Update as Order Delivered" class='badge badge-danger badge-pill py-2 px-2' *ngIf="rowData && rowData.status == 6" (click)="confirmModal.show()" >Cancel Order</span>
    ...
    <span style="cursor: pointer;" title="Update as Order refunded" class='badge badge-success badge-pill py-2 px-2' *ngIf="rowData && rowData.status == 17" (click)="confirmModal.show()" >Order refunded</span>
    ...
    <span style="cursor: pointer;"  class='badge badge-success badge-pill py-2 px-2' *ngIf="rowData && rowData.status == 18" >{{rowData.refundStatus}}</span>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content cases-model">
                ...
                <div class="modal-body">
                    <h6 *ngIf="rowData && rowData.status == 1 || rowData.status==3 || rowData.status==6"> Are you sure! Do you want to Cancel The Order </h6>
                </div>
