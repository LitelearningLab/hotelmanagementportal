...
const createNotification=async(req,res)=>{
    try {
        ...
        if(action==="create"){
            ...
            savedata.imagename=req.file.originalname
            let result=new notificationSchema(savedata)
            let data=await result.save()
            let url = process.env.url+data.image
            const correctedUrl = url.replace(/\\/g, '/');
            let messagedata={title:req.body.title,message:req.body.message,image:correctedUrl}
...
const smsSave=async(req,res)=>{
    try {
        console.log(req.body)
        let reqdata=req.body
        let data={
            apikey:reqdata.twilio.apikey,
            clientid:reqdata.twilio.clientid,
            mode:reqdata.twilio.mode,
            gateway:"taggsms"
        }
        let result = await smsSchema.updateMany({gateway:"taggsms"},{$set:{...data}},{upsert:true})
        res.send({message:"updated Successfully ",status:true})
    } catch (error) {
        console.log(error)
        res.status(500).send({message:"somthing went wrong",status:false})
...
const getSmsSetting=async(req,res)=>{
    try {
        let data=await smsSchema.findOne({gateway:"taggsms"})
        if(data){
            res.send({message:"fetched data successfully", twilio:data})
        }
